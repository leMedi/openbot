...
The MVP schema has these tables. Model references use Pi's canonical
`provider/model-id` form, splitting only at the first slash because model IDs
may themselves contain slashes. The singleton `setting` row stores the default
agent and orchestrator model references. Agent turns inherit the default when
they have no override; group decision turns use the orchestrator model.
The original unqualified seed values remain a legacy migration exception:
runtime resolution accepts one only when it identifies exactly one known model,
and the next settings save persists its canonical reference.
...
Pi provider credentials and cached model catalogs are server-owned files under
`$OPENBOT_DATA_DIR/pi-agent`. Provider definitions and login interactions come
from the installed Pi SDK. Completed credentials persist in Pi's `auth.json`;
in-progress browser login flows are runtime-only and fail on server restart.
...
runs Pi's completion/tool loop. User-visible delivery follows the
...
group-targeted turn queued per user post, and a Pi-backed group orchestrator
using the installation's configured orchestrator model. It selects the needed
members and queues agent-targeted child turns with parent/round/position
linkage; malformed selections fall back to explicit mention matching or the
full member roster. Member answers carry `sender_agent_id` in the shared
transcript only. Selection from broader recent room context and further
bounded rounds remain future work.
...
- `packages/agent/src/prompt/assembly.ts`: private/group prompt and Pi session
  preparation.
- `packages/agent/src/ai.ts`: shared Pi provider, credential, model catalog,
