...
import { deleteManagedFileIfUnreferenced } from './files'
...
export const cleanTargets = ['bots', 'conversations', 'memory', 'mcps'] as const
...
export async function cleanData(targets: ReadonlySet<CleanTarget>) {
  ...
  if (targets.has('bots')) {
    const avatars = await db.select({ fileId: agents.avatarFileId }).from(agents)
